...
final class MarksController {
    public function subjectStudents(Request $r,array $u):never{
        ...
        $sql="SELECT s.StudentID id,s.StudentName name,s.AdmissionNo admissionNo,ss.RollNo rollNo,s.FatherName fatherName,
                     m.CBSEExamMarksEntryID marksEntryId,m.TermOptionID termOptionId,m.SubSubjectID subSubjectId,m.StudentID marksStudentId,
                     CAST(m.MaxMarks AS float) maxMarks,CAST(m.MinMarks AS float) minMarks,CAST(m.ObtainMarks AS float) obtainedMarks,CAST(m.Percentage AS float) percentage,
                     m.IsAbsent isAbsent,m.IsMedical isMedical,m.Serial serial,m.OwnerSessionID ownerSessionId,m.IsDef isDefault
    ...
    public function studentWise(Request $r,array $u,int $student):never{
        ...
        $sql="SELECT s.CBSEExamSubjectID subjectId,s.CBSEExamSubject subjectName,
                     m.CBSEExamMarksEntryID marksEntryId,m.TermOptionID termOptionId,map.CBSEExamSubSubjectID subSubjectId,m.StudentID studentId,
                     CAST(m.MaxMarks AS float) maxMarks,CAST(m.MinMarks AS float) minMarks,CAST(m.ObtainMarks AS float) obtainedMarks,CAST(m.Percentage AS float) percentage,
                     m.IsAbsent isAbsent,m.IsMedical isMedical,m.Serial serial,m.OwnerSessionID ownerSessionId,m.IsDef isDefault
              FROM ApiTeacherAssignment a JOIN CBSEExamSubject s ON s.CBSEExamSubjectID=a.SubjectID
              CROSS APPLY(SELECT TOP 1 ss.CBSEExamSubSubjectID FROM CBSEExamSubSubject ss WHERE ss.CBSEExamSubjectID=s.CBSEExamSubjectID ORDER BY ss.CBSEExamSubSubjectID) map
              LEFT JOIN CBSEExamMarksEntry m ON m.SubSubjectID=map.CBSEExamSubSubjectID AND m.StudentID=? AND m.TermOptionID=? AND m.OwnerSessionID=?
    ...
    private function write(array $u,int $class,int $section,int $term,float $commonMax,array $rows,callable $subjectOf):never{
        if(!$rows)Response::error('marks must be a non-empty array.',422,'VALIDATION_ERROR');$session=SchoolService::currentSessionId();$db=Database::connection();$db->beginTransaction();$savedRows=[];
        try{foreach($rows as $x){Validator::required($x,['studentId']);$student=Validator::id($x['studentId'],'studentId');$subject=$subjectOf($x);SchoolService::assertAssignment($u['ApiUserID'],$class,$section,$subject);$sub=SchoolService::subSubjectId($subject);$max=(float)($x['maxMarks']??$commonMax);$abs=(bool)($x['isAbsent']??false);$med=(bool)($x['isMedical']??false);
            ...
            $pct=$obt===null?null:round($obt*100/$max,2);$exists=$db->prepare('SELECT COUNT(*) FROM CBSEExamMarksEntry WITH (UPDLOCK,HOLDLOCK) WHERE StudentID=? AND OwnerSessionID=? AND TermOptionID=? AND SubSubjectID=?');$exists->execute([$student,$session,$term,$sub]);
            if((int)$exists->fetchColumn()>0){$db->prepare('UPDATE CBSEExamMarksEntry SET MaxMarks=?,ObtainMarks=?,Percentage=?,IsAbsent=?,IsMedical=? WHERE StudentID=? AND OwnerSessionID=? AND TermOptionID=? AND SubSubjectID=?')->execute([$max,$obt,$pct,(int)$abs,(int)$med,$student,$session,$term,$sub]);}
            else{$db->prepare('INSERT INTO CBSEExamMarksEntry(TermOptionID,SubSubjectID,StudentID,MaxMarks,MinMarks,ObtainMarks,Percentage,IsAbsent,IsMedical,OwnerSessionID,IsDef) VALUES(?,?,?,?,0,?,?,?,?,?,0)')->execute([$term,$sub,$student,$max,$obt,$pct,(int)$abs,(int)$med,$session]);}
            $savedRows[$student.':'.$subject]=['studentId'=>$student,'subjectId'=>$subject,'maxMarks'=>$max,'obtainedMarks'=>$obt,'percentage'=>$pct,'isAbsent'=>$abs,'isMedical'=>$med];
        }$db->commit();}catch(\Throwable $e){if($db->inTransaction())$db->rollBack();throw$e;}Response::success(['saved'=>count($savedRows),'classId'=>$class,'sectionId'=>$section,'termOptionId'=>$term,'marks'=>array_values($savedRows)]);
    }
