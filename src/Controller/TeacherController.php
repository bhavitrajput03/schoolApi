...
final class TeacherController {
    public function dashboard(Request $r,array $u):never{$s=SchoolService::currentSessionId();$st=Database::connection()->prepare("SELECT COUNT(DISTINCT CONCAT(ClassID,':',SectionID)) FROM ApiTeacherAssignment WHERE ApiUserID=? AND OwnerSessionID=? AND IsActive=1");$st->execute([$u['ApiUserID'],$s]);Response::success(['teacher'=>['id'=>$u['ApiUserID'],'name'=>$u['DisplayName'],'role'=>$u['Role']],'academicSessionId'=>$s,'assignedClassSections'=>(int)$st->fetchColumn()]);}
