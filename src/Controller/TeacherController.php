...
final class TeacherController {
    public function dashboard(Request $r,array $u):never{$session=SchoolService::currentSession();$st=Database::connection()->prepare("SELECT COUNT(DISTINCT CONCAT(ClassID,':',SectionID)) FROM ApiTeacherAssignment WHERE ApiUserID=? AND OwnerSessionID=? AND IsActive=1");$st->execute([$u['ApiUserID'],$session['id']]);Response::success(['teacher'=>['id'=>$u['ApiUserID'],'name'=>$u['DisplayName'],'role'=>$u['Role']],'academicSessionId'=>$session['id'],'academicSession'=>$session,'assignedClassSections'=>(int)$st->fetchColumn()]);}
