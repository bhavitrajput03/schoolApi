...
final class SchoolRegistry {
    ...
    public static function find(string $schoolCode):?array{
        ...
        $st=RegistryDatabase::connection()->prepare('SELECT TOP 1 ConnectionString FROM dbo.SchoolConnection WHERE UPPER(LTRIM(RTRIM(SchoolCode)))=?');$st->execute([$schoolCode]);$row=$st->fetch();
