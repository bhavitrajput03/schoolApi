...
final class SchoolService {
    public static function currentSessionId(): int {
        $id=Database::connection()->query("SELECT TOP 1 OwnerSessionID FROM OwnerSession ORDER BY CASE WHEN CAST(GETDATE() AS date) BETWEEN CAST(StartDate AS date) AND CAST(EndDate AS date) THEN 0 ELSE 1 END,EndDate DESC")->fetchColumn();
        if(!$id)Response::error('No academic session configured.',503,'SESSION_NOT_CONFIGURED');return(int)$id;
    }
