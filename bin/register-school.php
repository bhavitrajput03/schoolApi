...
try{$update=$db->prepare('UPDATE dbo.SchoolConnection SET SchoolCode=?,ConnectionString=? WHERE LTRIM(RTRIM(SchoolCode)) COLLATE Latin1_General_100_CI_AS=? COLLATE Latin1_General_100_CI_AS');$update->execute([$code,trim($connectionString),$code]);if($update->rowCount()===0)$db->prepare('INSERT INTO dbo.SchoolConnection(SchoolCode,ConnectionString) VALUES(?,?)')->execute([$code,trim($connectionString)]);$db->commit();}catch(\Throwable $e){if($db->inTransaction())$db->rollBack();throw$e;}echo"School SQL connection saved successfully.\n";
