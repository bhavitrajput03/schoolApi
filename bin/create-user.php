...
Database::useSchoolCode($school);$db=Database::connection();$st=$db->prepare('SELECT SchoolBranchID FROM ApiSchool WHERE SchoolCode=? AND IsActive=1');$st->execute([strtoupper($school)]);$branch=$st->fetchColumn();if(!$branch){fwrite(STDERR,"Unknown school code.\n");exit(1);}
